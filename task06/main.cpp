...
auto sample_hemisphere(
  ...
  float pdf=nrm.dot(local_to_world_vector_transformation(nrm) *dir_loc)/M_PI;
      ...
      if (res) { // ambient occlusion computation
        ...
        for (unsigned int i_sample = 0; i_sample < num_sample_ao; ++i_sample) {
          ...
          if (!res1) { // if the ray doe not hit anything
            sum += dir.dot(nrm)/(pdf*M_PI); // Problem 3: This is a bug. write some correct code (hint: use `dir.dot(nrm)`, `pdf`, `M_PI`).
          }
        }
