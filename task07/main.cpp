  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit0_brdf*(hit1_rad*(hit1_normal.dot((hit0_pos-hit1_pos).normalized()))*hit0_refl.dot(hit0_normal)/(nsample*hit0_pdf)); // replace this with some code
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float w1=hit0_pdf_brdf_sample/(hit0_pdf_brdf_sample+hit0_pdf_light_sample);
        float rad=w1*hit0_brdf*(hit1_rad*(hit1_normal.dot((hit0_pos-hit1_pos).normalized()))*hit0_refl.dot(hit0_normal)/(nsample*hit0_pdf_brdf_sample));
